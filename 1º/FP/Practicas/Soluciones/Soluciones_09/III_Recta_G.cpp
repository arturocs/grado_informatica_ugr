...
public:
	...
	{		
		if (Hay_Error_en_coeficientes(valor_A, valor_B)) {
			
			// La situación es comprometida: el objeto que se está creando 
			// no debería quedar en una situación "inválida" ¿qué hacer? 
			//	1) Fijar los coeficientes a un valor por defecto :-(

			A = B = 1;		// El valor por defecto es 1, p.e.
			C = valor_C;

			// 	2) Abortar la ejecución del programa :-(
			//
			// Ninguna estrategia es buena: el constructor nunca debió 
			// ejecutarse. 
			//
			// Si no se hace nada, el objeto creado tiene valores basura :-(
		}
		
		else {	// Fijar sin problemas los valores de los coeficientes
	...
	{	
		if (Hay_Error_en_coeficientes(valor_A, valor_B)) {
			...
			// La situación es comprometida: el objeto no debería quedar en 
			// una situación "inválida" ¿qué hacer? 
			//	1) Fijar los coeficientes a un valor por defecto :-(
			...
			// 	2) Abortar la ejecución del programa :-(
			//
			// Ninguna estrategia es buena: este método nunca debió 
			// ejecutarse. 
			//
			// Si no se hace nada, el objeto creado no se modifica :-( 
		}
...
private:
	...
	//
	// Recibe: 	coef_A, Coeficiente de x
	// 			coef_B, Coeficiente de y
	// Devuelve: true, si coef_A=0 y coef_B=0
	
	bool Hay_Error_en_coeficientes (double coef_A, double coef_B) 
	{
...
int main (void)
{
	...
	// CUIDADO: NO hemos asegurado el cumplimiento de las precondiciones
