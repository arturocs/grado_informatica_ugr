...
/*
	...
	4) Se aplica una retención fiscal y que ésta es la misma para los 
	   fabricantes y el diseñador. 
	5) Una vez que se establezca la retención fiscal, ésta no cambia.

	El programa lee el valor de los ingresos totales y la retención a 
	aplicar y calcula los salarios brutos y netos de los fabricantes 
...
private:
	...
	// Constante a nivel de objeto
	// PRE: 0 <= PORC_RETENCION_FISCAL <= 100
   const double PORC_RETENCION_FISCAL; 
...
public:   
	...
	// PRE: 0 <= porc_retencion <= 100

	Nomina (double ingresos_totales_nomina, double porc_retencion) : 
			PORC_RETENCION_FISCAL (porc_retencion)
	{
	...
	double GetPorcRetencionFiscal (void)
	{
      return (PORC_RETENCION_FISCAL);
	}
...
int main (void)
{
	...
	do {
		cout << "Introduzca porcentaje de retención: ";
		cin >> porc_retencion;
	} while ((porc_retencion < 0) || (porc_retencion > 100));

	
	// Durante la ejecución del programa, el objeto "una_nomina" tendrá 
	// un valor constante de retención. Además, todos los objetos de la 
	// clase "nomina" tendrán el mismo valor para la constante, cuyo valor 
	// le lee el tiempo de ejecución.

	// Creación de un objeto "Nomina" --> constructor
  
	Nomina nomina_empresa (ingresos_una_empresa, porc_retencion);
	...
	cout << "\tRetencion (" << setw(5) << setprecision(2) 
		 << nomina_empresa.GetPorcRetencionFiscal() << " %) = " 
