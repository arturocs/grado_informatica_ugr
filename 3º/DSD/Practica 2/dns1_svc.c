...
/*
	case cliente:
		_xdr_argument = (xdrproc_t) xdr_wrapstring;
		_xdr_result = (xdrproc_t) xdr_wrapstring;
		local = (char *(*)(char *, struct svc_req *)) _cliente_1;
		break;
*/
	default:
...
main (int argc, char **argv)
{
	...
	if (!svc_register(transp, DNS1, DNSVER1, dns1_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (DNS1, DNSVER1, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, DNS1, DNSVER1, dns1_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (DNS1, DNSVER1, tcp).");
		exit(1);
	}
