...
int main(int argc, char *argv[]) {
  ...
  for (int argumento = 3; argumento < argc; argumento++) {
    ...
    chrono::high_resolution_clock::time_point t_antes =
        chrono::high_resolution_clock::now();
    mergesort(T, n);
    chrono::high_resolution_clock::time_point t_despues =
        chrono::high_resolution_clock::now();
    unsigned long t_ejecucion =
        chrono::duration_cast<std::chrono::microseconds>(t_despues - t_antes)
            .count();
    cerr << "\tTiempo de ejec. (us): " << t_ejecucion << " para tam. caso " << n
         << endl;
