...
const int num_filosofos = 5, num_procesos = 2 * num_filosofos + 1,
          id_camarero = 10, etiq_levantarse = 2, etiq_sentarse = 1;
...
void funcion_filosofos(int id) {
  ...
  while (true) {
    if (id == 0) {
      ...
      MPI_Ssend(&val, 1, MPI_INT, id_ten_izq, 0, MPI_COMM_WORLD);

      cout << "Filósofo " << id << " solicita ten. der." << id_ten_der << endl;
      // ... solicitar tenedor derecho
      MPI_Ssend(&val, 1, MPI_INT, id_ten_der, 0, MPI_COMM_WORLD);
...
void camarero() {
  int etiq_auxiliar, val, sentados = 0;
  MPI_Status estado;
  while (true) {
    if (sentados < num_filosofos - 1) {
      etiq_auxiliar = MPI_ANY_TAG;
    } else {
      etiq_auxiliar = etiq_levantarse;
    }

    MPI_Recv(&val, 1, MPI_INT, MPI_ANY_SOURCE, etiq_auxiliar, MPI_COMM_WORLD, &estado);
    if (estado.MPI_TAG == etiq_sentarse) {
      sentados++;
      MPI_Ssend(&val, 1, MPI_INT, estado.MPI_SOURCE, etiq_sentarse,
                MPI_COMM_WORLD);
