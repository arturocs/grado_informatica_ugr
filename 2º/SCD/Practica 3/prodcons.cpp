...
const int np = 4, nc = 5, id_buffer = np, etiq_productor = 1,
          etiq_consumidor = 2, num_procesos_esperado = np + nc + 1,
          tam_vector = 10, num_items = np * nc;
...
int producir(int prod) {
  ...
  contador++;
  cout << "Productor " << prod << " ha producido valor " << contador << endl
       << flush;
  return contador;
}
// ---------------------------------------------------------------------
...
void funcion_productor(int prod) {
  for (unsigned int i = 0; i < num_items / np; i++) {
    ...
    // enviar valor
    cout << "Productor " << prod << " va a enviar valor " << valor_prod << endl
         << flush;
...
void consumir(int valor_cons, int cons) {
  // espera bloqueada
  sleep_for(milliseconds(aleatorio<10, 200>()));
  cout << "Consumidor " << cons << " ha consumido valor " << valor_cons << endl
       << flush;
}
// ---------------------------------------------------------------------
...
void funcion_consumidor(int cons) {
  ...
  for (unsigned int i = 0; i < num_items / nc; i++) {
    ...
    consumir(valor_rec, cons);
  }
}
// ---------------------------------------------------------------------

void funcion_buffer() {
  int buffer[tam_vector], valor, primera_libre = 0, primera_ocupada = 0,
                                 num_celdas_ocupadas = 0, id_emisor_aceptable;
  MPI_Status estado;
  for (unsigned int i = 0; i < num_items * 2; i++) {
    if (num_celdas_ocupadas == 0) {
      id_emisor_aceptable = etiq_productor;
    } else if (num_celdas_ocupadas == tam_vector) {
      id_emisor_aceptable = etiq_consumidor;
    } else {
      id_emisor_aceptable = MPI_ANY_TAG;
    }
    MPI_Recv(&valor, 1, MPI_INT, MPI_ANY_TAG, id_emisor_aceptable, MPI_COMM_WORLD,
             &estado);
...
int main(int argc, char *argv[]) {
  ...
  if (num_procesos_esperado == num_procesos_actual) {
    if (id_propio < np)   // si mi ident. es el del productor
