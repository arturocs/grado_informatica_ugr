/* SumaVectoresC.c
 ...
 Para compilar usar (-lrt: real time library, es posible que no sea necesario
 usar -lrt): gcc  -O2  SumaVectores.c -o SumaVectores -lrt gcc  -O2 -S
 SumaVectores.c -lrt
...
// ... tres defines siguientes puede estar descomentado): #define VECTOR_LOCAL
...
int main(int argc, char **argv) {
  ...
  unsigned int N =
      atoi(argv[1]); // M�ximo N =2^32-1=4294967295 (sizeof(unsigned int) = 4 B)
  ...
  double *v1, *v2, *v3;
  v1 = (double *)malloc(N *
                        sizeof(double)); // malloc necesita el tama�o en bytes
...
#pragma omp parallel for
  ...
  printf("Primeros elementos de v1, v2 y v3: %f, %f, %f \n", v1[0], v2[0],
         v3[0]);
  printf("Últimos elementos de v1, v2 y v3: %f, %f, %f \n", v1[N - 1],
         v2[N - 1], v3[N - 1]);
